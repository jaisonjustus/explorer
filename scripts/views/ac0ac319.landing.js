define(['jquery', 'underscore', 'backbone', 'store', 'access'], function($, _, Backbone, Store, Access) {
  ...
  return Backbone.View.extend({
    ...
    , getToken: function(){
      ...
      var xhr = $.ajax({url:url, data:{}, type:'POST', contentType:'application/json', dataType:'json'})
    ...
    , onClickSigninBtn  : function(e){
      ...
      var xhr = $.ajax({url:url, data:data, type:'POST', contentType:'application/json', dataType:'json'})
        .success(_.bind(function(res){ 
          if (typeof(res) === 'string'){
            res = JSON.parse(res);
          }
